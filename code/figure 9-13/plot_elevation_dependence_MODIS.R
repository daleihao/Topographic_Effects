...
for (i in 1:6)
{
 ...
 all_data <- melt(all_data, id.vars = c('Elevation'))
  ...
  all_data <- all_data %>% filter(value>-100)
  plot_scale_difference <- ggplot(all_data, aes(x = variable, y = value, fill = Elevation)) + ylab('d')  +ggtitle(code) +
    stat_boxplot(geom ='errorbar', width = 0.6, size = 0.5, show.legend = FALSE, position = position_dodge(0.9)) +
    geom_boxplot(size = 0.3, show.legend = TRUE, width = 0.6, outlier.shape = 1,outlier.size = 0.1, outlier.alpha = 0.1, position = position_dodge(0.9)) + 
    theme_classic() + 
    ...
    scale_fill_brewer(palette="Dark2", labels = c('<1.5 km','1.5-2.5 km','2.5-3.5 km','3.5-4.5 km','>4.5 km')) +
    ylim(c(-0.2,0.2))+
  ...
  if(i == 1)
  {
    plot_scale_difference <- plot_scale_difference + 
      ylim(c(-0.1,0.1)) + 
      ylab(expression("|"~delta[TOP]~"|"~-~"|"~delta[PP]~"|"))
  }
  else if(i==2)
  { plot_scale_difference <- plot_scale_difference + 
    ylim(c(-0.1,0.1)) + 
  ...
  { plot_scale_difference <- plot_scale_difference + 
    ylim(c(-1,1)) + 
    ylab(expression("|"~delta[TOP]~"|"~-~"|"~delta[PP]~"| (K)"))}
  else if(i==5)
  { plot_scale_difference <- plot_scale_difference + 
    ylim(c(-0.5,0.5)) + 
    ylab(expression("|"~delta[TOP]~"|"~-~"|"~delta[PP]~"| (K)"))}
  ...
  { plot_scale_difference <- plot_scale_difference + 
    ylim(c(-1,1)) + 
    ylab(expression("|"~delta[TOP]~"|"~-~"|"~delta[PP]~"| ("~W/m^2~")"))}
  ggsave(paste0('MODIS_', name, '_',scale,".tiff"), plot = plot_scale_difference, width = 15, height = 7, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")
}
