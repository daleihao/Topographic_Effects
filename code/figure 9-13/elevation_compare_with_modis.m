...
for res = 1:1
    ...
    for i = 1:4
        ...
        if(i==4)
            save('MODIS_ST_d.mat','group_data');
        end
        %% plot 1
    ...
    for i = 1:4
        ...
        if(i==4)
            save('MODIS_ST_n.mat','group_data');
        end
        %% plot 1
    ...
    for i = 1:4
        ...
        filters = mean_elevation<1500 & mean_slope>0;
        b1 = (BSA_delta_difference(filters));
        b1(notop_BSA(filters)<=0)=-999;
        filters = mean_elevation>=1500 & mean_elevation<2500 & mean_slope>0;
        b2 = (BSA_delta_difference(filters));
        b2(notop_BSA(filters)<=0)=-999;
        filters = mean_elevation>=2500 & mean_elevation<3500 & mean_slope>0 ;
        b3 = (BSA_delta_difference(filters));
        b3(notop_BSA(filters)<=0)=-999;
        filters = mean_elevation>=3500 & mean_elevation<4500 & mean_slope>0 ;
        b4 = (BSA_delta_difference(filters));
        b4(notop_BSA(filters)<=0)=-999;
        filters = mean_elevation>=4500 & mean_slope>0;
        b5 = (BSA_delta_difference(filters));
        b5(notop_BSA(filters)<=0)=-999;
        bdata = [b1; b2; b3; b4; b5];
        group = [1*ones(size(b1)); 2*ones(size(b2)); 3*ones(size(b3)); 4*ones(size(b4)); 5*ones(size(b5))];
