...
codes = c(expression(bold('(a) Direct albedo')),expression(bold('(b) Diffuse albedo')), expression(bold('(c) Snow cover fraction (%)')), expression(bold('(d) Daytime surface temperature (K)')), expression(bold('(e) Nighttime surface temperature (K)')),expression(bold("("~f~") Latent heat flux ("~W/m^2~")")));
for (i in 1:6)
{
  code = codes[i];
  ...
  plot_scale_difference <- ggplot(all_data, aes(x =Elevation , y = Winter, fill = Elevation)) + xlab('Elevation (km)')  +ggtitle(code) +
...
ggsave(paste0('BOXPLOT_MODIS_',scale,".tiff"), plot = plot_figure, width = 28, height = 14, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")
