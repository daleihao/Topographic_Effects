...
for res = 1:5
    ...
    for i = 1:1
        ...
        switch res
            case 1
                scale_1_all = Albedo_abs_difference;
            case 2
                scale_2_all = Albedo_abs_difference;
            case 3
                scale_3_all = Albedo_abs_difference;
            case 4
                scale_4_all = Albedo_abs_difference;
            case 5
        ...
        if row_tmp<3
            if(col_tmp==1)
                plot_global_map(lats, lons, Albedo_abs_difference, -1, 1,strcat(index, scales{res}), colors_2,1, 0);
            else
                plot_global_map(lats, lons, Albedo_abs_difference, -1, 1,strcat(index, scales{res}), colors_2,0, 0);
            end
        else
            if(col_tmp==1)
                plot_global_map(lats, lons, Albedo_abs_difference, -1, 1,strcat(index, scales{res}), colors_2,1, 1);
            else
                plot_global_map(lats, lons, Albedo_abs_difference, -1, 1,strcat(index, scales{res}), colors_2,0, 0);
            end
        end        
...
hold on
[f_tmp,y_tmp] = histcounts(scale_5_all(:), 60,'BinLimits',[-1 1],'Normalization','probability');
y_tmp = (y_tmp(1:(end-1))+1/30/2);
yyaxis right
plot(y_tmp,f_tmp*100, 'r-','LineWidth',0.8)

[f_tmp,y_tmp] = histcounts(scale_4_all(:), 60,'BinLimits',[-1 1],'Normalization','probability');
y_tmp = (y_tmp(1:(end-1))+1/30/2);
yyaxis right
plot(y_tmp,f_tmp*100, 'y-','LineWidth',0.8)
[f_tmp,y_tmp] = histcounts(scale_3_all(:), 60,'BinLimits',[-1 1],'Normalization','probability');
y_tmp = (y_tmp(1:(end-1))+1/30/2);
yyaxis right
plot(y_tmp,f_tmp*100, 'c-','LineWidth',0.8)
[f_tmp,y_tmp] = histcounts(scale_2_all(:), 60,'BinLimits',[-1 1],'Normalization','probability');
y_tmp = (y_tmp(1:(end-1))+1/30/2);
yyaxis right
plot(y_tmp,f_tmp*100, 'b-','LineWidth',0.8)
[f_tmp,y_tmp] = histcounts(scale_1_all(:), 60,'BinLimits',[-1 1],'Normalization','probability');
y_tmp = (y_tmp(1:(end-1))+1/30/2);
yyaxis right
...
print(gcf, '-dsvg', '-r300', '../revised_figures/figure_S4_surface_temperature.svg')
close all
