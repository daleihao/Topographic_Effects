...
# import data
scale = 'r0125';
variables_name = c('albedo','albedo', 'albedo', 'albedo');
codes = c('(a) Winter','(b) Spring', '(c) Summer', '(d) Autumn');
for (i in 1:4)
{
  code = codes[i];
  ...
  {all_data <- all_data %>% 
    select(Elevation, Spring) %>% 
    filter(Elevation>1)} 
  else if(i==3)
  {all_data <- all_data %>% 
    select(Elevation, Summer) %>% 
    filter(Elevation>1)} 
  else if(i==4)
  {all_data <- all_data %>% 
    select(Elevation, Autumn) %>% 
    filter(Elevation>1)} 
  all_data[['Elevation']] <- factor(all_data[['Elevation']])
  colnames(all_data) <-c('Elevation','Winter');
...
ggsave(paste0('different_season_BOXPLOT_TOP_PP_',scale,".tiff"), plot = plot_figure, width = 20, height = 14, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")
