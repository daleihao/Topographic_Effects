...
# import data
scale = 'r0125';
variables_name = c('net_solar_radiation','albedo', 'snow_cover', 'temperature', 'sensible_heat_flux','latent_heat_flux');
codes = c('(a) Net solar radiation','(b) Land surface albedo', '(c) Snow cover fraction', '(d) Surface temperature', '(e) Sensible heat flux','(f) Latent heat flux');
for (i in 1:6)
{
  code = codes[i];
...
ggsave(paste0('revised_BOXPLOT_TOP_PP_',scale,".tiff"), plot = plot_figure, width = 28, height = 14, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")
