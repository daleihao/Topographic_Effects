...
scales = c('r0125','r025', 'r05', 'f09', 'f19');
codes = c('(a) r0125','(b) r025', '(c) r05', '(d) f09', '(e) f19');
for (i in 1:5)
{
  scale = scales[i];
  code = codes[i];
  
all_datas <- readMat(paste0('albedo_' , scale, '.mat'));
...
ggsave(paste0('BOXPLOT_TOP_PP_all_scales',".tiff"), plot = plot_figure, width = 28, height = 14, units = "cm", dpi = 300, limitsize = FALSE, compression = "lzw")
