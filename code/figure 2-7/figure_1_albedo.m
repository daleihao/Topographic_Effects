...
for res = 1:1
    ...
    for i = 1:4
        ...
        %% plot 1
        ax1 = subplot('position', [0.06 + 0.9/4*(i-1) 0.65 0.2 0.3])
        colormap(ax1, colors);
        hold on
        if(i==1)
            plot_global_map(lats, lons, Albedo_PP, 0, 1,strcat(index, SeasonsNames{i}), colors,1, 0);
        else
            plot_global_map(lats, lons, Albedo_PP, 0, 1,strcat( index, SeasonsNames{i}), colors,0, 0);
        end
        ...
        ax2 = subplot('position', [0.06 + 0.9/4*(i-1) 0.35 0.2 0.3])
        colormap(ax2, colors_2);
        hold on
        if(i==1)
            plot_global_map(lats, lons, Albedo_abs_difference, -0.1, 0.1,"", colors_2,1, 0);
        else
            plot_global_map(lats, lons, Albedo_abs_difference, -0.1, 0.1,"", colors_2,0, 0);
        end
        ...
        hold on
        if(i==1)
            plot_global_map(lats, lons, Albedo_rel_difference*100, -20, 20,"", colors_2,1, 1);
        else
            plot_global_map(lats, lons, Albedo_rel_difference*100, -20, 20,"", colors_2,0, 1);
        end
    ...
    print(gcf, '-dsvg', '-r300', '../improved_figures/figure_10_albedo.svg')
